...
**Goal:** 다섯 핵심 세법에서 10개씩 고른 50개 쟁점을 `질문 → 새 사실 한 조각 → 관련 법령 → 답·해설`의 순차 흐름으로 읽는 별도 모바일 우선 정적 사이트를 만들고 GitHub Pages 게시 가능한 번들을 만든다.

**Architecture:** `content/`는 검증된 공개 학습 데이터, `scripts/bootstrap_content.py`는 기존 `tax-study` 공개 원천에서 50개 큐레이션 데이터를 한 번 추출하며, `scripts/build.py`는 그 데이터만 읽어 `docs/`에 독립 HTML을 생성한다. 각 쟁점은 순서가 있는 `steps` 배열로 표현하고, 한 단계의 질문·새 사실·관련 조문·답·증빙·판례 연결을 함께 보관한다. 런타임 JavaScript는 법률·쟁점 필터만 담당하고, 단계와 원문은 브라우저 기본 `details`만으로 JavaScript 없이 읽힌다. 기존 `tax-study`의 원장·템플릿·HTML은 수정하지 않는다.
...
- [x] `bootstrap_content.py`가 `../tax-study/data/laws/learning.json`, 다섯 법률 JSON, 선택된 50개 lesson Markdown, 판례 인덱스를 읽어 issue ID를 중복 없이 모은다. 각 issue에는 법률코드, 제목, 핵심질문, 순차 `steps`, 공식 출처, source lesson provenance를 넣는다. 각 step은 질문·새 사실 한 조각·관련 article ID·답·해설을 필수로 하고, 증빙·판례·회계/조정은 해당 단계에만 선택적으로 붙인다.
...
- [x] 기존 원고의 전체 사례·판단지도·결론 반복을 순차 step으로 재편한다. 화면용 내부 ID(`R1`, `R2`)는 제거하고, 학습자에게는 새 사실과 직전 결론만 보인다. 동일 사실이 두 step에 다시 나오면 추출 단계에서 실패시킨다.
...
- [x] 쟁점 리더는 `오늘의 질문 → 단계 1(새 사실 → 관련 조문 → 답·해설) → 단계 2 ... → 판례·조사·회계 연결 → 사실이 달라지면 → 마지막 정리` 순서로 렌더한다. 전체 사례를 앞에 한 번에 제시하지 않고, 각 단계의 새 사실만 한 번씩 보여 준다. 시험형 요약은 독립 연습 데이터가 없으면 만들지 않는다.
- [x] 각 step은 질문을 `summary`로 먼저 노출하고, 펼친 뒤 `새 사실`, `관련 조문(기본 접힘)`, `답과 해설` 순서를 지킨다. 한 번에 여러 사실을 기억하게 만드는 `R1/R2` 표와 추상적인 판단지도 블록은 만들지 않는다.
...
- [x] 순차 step은 모바일에서 세로로 이어지는 번호와 질문 제목만 먼저 보여 주고, 새 사실·관련 조문·답을 한 단계씩 펼치게 한다. 원문은 해당 step의 `details` 안에서만 기본 접힘으로 제공한다.
...
- [x] 테스트는 모든 issue에 2개 이상의 순차 step과 필수 필드(question/new_fact/legal_refs/answer)가 있는지, 생성된 DOM이 질문→사실→법령→답 순서인지, 같은 사실·사례 블록이 반복되지 않는지 검증한다.
...
- [x] `docs/` 번들을 commit·push하고 Pages source가 `main:/docs`인 공개 주소에서 새 HTML을 확인한다. remote main SHA `5a301b8`와 게시된 순차 화면이 일치할 때만 공개 URL을 전달한다.
