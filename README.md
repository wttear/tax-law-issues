...
국세기본법·국세징수법·법인세법·소득세법·부가가치세법에서 10개씩 고른 50개 쟁점을 읽는 Next.js 정적 사이트입니다. 각 쟁점은 `사실관계 전문 → 질문별 판단 → 관련 법령 원문 → 세무조사·회계 연결 → 판례 → 조건 변경 → 마지막 정리` 순서로 이어집니다.
...
- `content/issue-selection.json`: 법령별 10개 선정 목록
