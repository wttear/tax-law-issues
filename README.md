...
국세기본법·국세징수법·법인세법·소득세법·부가가치세법에서 10개씩 고른 50개 쟁점을 읽는 별도 정적 사이트입니다. 각 쟁점은 `답을 보기 전 → 현행 법령 원문과 기준일 → 초보자 해설 → 판단 지도 → 합성 사례와 증빙 → 관련 판례 → 필요한 경우 회계·세무조정 → 출처` 순서로 구성됩니다.
...
- `content/issues.json`: 50개 쟁점과 원고의 학습 섹션
...
현재 공개 시안에서는 기존 원고의 일반적인 시험형 섹션을 독립 문제 데이터로 오인하지 않도록 읽기 화면에서 제외했습니다. 실제 기출 원문·정답·해설 데이터가 별도 검증되면 선택 모듈로 추가할 수 있습니다.
...
그 다음 저장소 Settings → Pages에서 `main` 브랜치의 `/docs` 폴더를 선택합니다. 이 작업공간에서 확인한 GitHub CLI 토큰은 만료되어 있으므로, 인증 전에는 공개 URL이 존재한다고 가정하지 않습니다.
